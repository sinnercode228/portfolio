...
Стили и скрипты лежат в самом файле, с других доменов ничего не грузится. Из репозитория подтягиваются обложки из [`assets/projects/`](assets/projects/) и шрифты Unbounded, Onest и JetBrains Mono из [`assets/fonts/`](assets/fonts/) (лицензия OFL, только кириллица и латиница, около 150 КБ). Оба языка есть в разметке, лишний прячет CSS, так что без JS видна русская версия. Язык (`?lang=en` или сохранённый) и тему (сохранённую или системную) скрипт в `<head>` ставит до первой отрисовки. Кнопки RU/EN и темы запоминают выбор, при переключении на EN в адрес дописывается `?lang=en`.
